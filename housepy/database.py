...
"""Taken from the Tornado framework, adapted to use housepy config and logging, and to handle duplicates"""
"""Adapted from https://github.com/facebook/tornado/blob/master/tornado/database.py August 3, 2010, ca2be3e99e863cc1395d"""
...
class Connection(object):
    ...
    def _execute(self, cursor, query, parameters):
        try:
            if 'tornado_sessions' not in query:
                log.info("%s" % (query % tuple(["'%s'" % str(p) for p in parameters])))
            return cursor.execute(query, parameters)
        except (OperationalError, IntegrityError, ProgrammingError), e:   # mapping errors to avoid MySQLdb imports
            if type(e.args) != int and e.args[0] == 1062:   # special exception for duplicates for easy catching
                log.info("-> duplicate")
                raise Duplicate()
            else:    
                self.close()
